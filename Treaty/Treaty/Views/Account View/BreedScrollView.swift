...
//                    .disabled(self.selectedBreed.isEmpty)
                }
            }
    ...
    @ViewBuilder
    func DogBreedRow(breed: Breed)-> some View {
        VStack(alignment: .leading, spacing: 15) {
//            Text(breed.value)
//                .font(.largeTitle.bold())
            
//            ForEach(breed.value,id: breed.index){_ in
                HStack(spacing: 10){
...
//                            .fill(breed.color.opacity(0.6))
//                            .frame(height: 20)
                        
//                        RoundedRectangle(cornerRadius: 4, style: .continuous)
//                            .fill(breed.color.opacity(0.4))
//                            .frame(height: 20)
//                            .padding(.trailing,80)
//                    }
                }
        ...
        .offsets(completion: { rect in
            ...
            if dogBreeds.indices.contains(breed.index){
                dogBreeds[breed.index].rect = rect
            }
            
            // Since Every Character moves up and goes beyond Zero (It will be like A,B,C,D)
            // So We're taking the last character
            if let last = dogBreeds.last(where: { char in
                char.rect.minY < 0
    ...
    func fetchCharacters(with values: [String]) -> [Breed] {
        let alphabets: String = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        var breeds: [Breed] = []
        
        breeds = values.compactMap({ breed -> Breed? in
            return Breed(value: breed)
        })
        ...
        for index in breeds.indices{
            breeds[index].index = index
            breeds[index].color = colors.randomElement()!
        }
